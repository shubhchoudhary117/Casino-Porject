...
import { MENUMASTER } from './master-menu';
import { MENUSUPER } from './super-menu';
import { MENUAGENT } from './agent-menu';
...
export class SidebarComponent implements OnInit, AfterViewInit {
  ...
  constructor(@Inject(DOCUMENT) private document: Document, private renderer: Renderer2,public router: Router,private route:ActivatedRoute,private authService:AuthService,private dataService:DataService) {
  ...
  ngOnInit(): void {
    console.log(this.router.url.startsWith(this.menuItems['link']))
  ...
  activateMenuItems() {
    ...
    let menuItemEl = null;
    for (let i = 0; i < links.length; i++) {
        if (window.location.href.includes(links[i]['pathname'])) {
            menuItemEl = links[i];
            break;
        }
    }
